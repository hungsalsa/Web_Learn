...
class Subcat extends \yii\db\ActiveRecord
{
    ...
    public function getCategoryParent($parent = 0, $level ="",$id_group=0){
        $result = Category::find()->asArray()->where('parent_id=:parent',['parent'=>$parent],'group_ID=:group',['group'=>$id_group])->all();
        // Tim tat ca cac 
        $level .=" -";
        foreach ($result as $key => $value) {
            if($parent == 0){
